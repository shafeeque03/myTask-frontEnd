...
function Sidebar(props) {
  ...
  const { permissions } = useSelector((state) => state.userReducer);
  ...
  function openChildren(id) {
    ...
    for (let i = 0; i < otherTabs.length; i++) {
      if (otherTabs[i].id !== id) {
        otherTabs[i].classList.remove("show");
  ...
  function onChangeDarkMode() {
    const theme = window.document.children[0].getAttribute("data-theme") === "light" ? "dark" : "light";
    window.document.children[0].setAttribute("data-theme", theme);
    setDarkLightMode(theme);
  }
    ...
    <div id="mainSideMenu" className={`sidebar px-4 py-4 py-md-5 me-0 ${isSidebarMini ? "sidebar-mini" : ""}`}>
      <div className="d-flex flex-column h-100">
        ...
        <ul className="menu-list flex-grow-1 mt-3">
          {menuData.filter(d => permissions.includes(d.name)).map((d, i) => {
            if (d.isToggled) {
              return (
                <li key={i}>
            ...
            return (
              <li key={i} className="collapsed">
                <a className={`m-link ${d.children.some(child => `/${child.routerLink[0]}` === activekey) ? "active" : ""}`} href="#!" onClick={(e) => { e.preventDefault(); openChildren(`menu-Pages${i}`); }}>
                  <i className={d.iconClass}></i>
                  <span>{d.name}</span>
                  <span className="arrow icofont-dotted-down ms-auto text-end fs-5"></span>
                </a>
                {d.children.length > 0 && (
                  <ul className="sub-menu collapse has-children" id={`menu-Pages${i}`}>
                    {d.children.map((data, ind) => {
                      ...
                      return (
                        <li key={ind}>
                          <Link className={activekey === `/${data.routerLink[0]}` ? "ms-link active" : "ms-link"} to={process.env.PUBLIC_URL + "/" + data.routerLink[0]}>
                            <span>{data.name}</span>
                          </Link>
                        </li>
                      );
